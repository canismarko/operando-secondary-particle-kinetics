...
where $c_p$ is the Li concentration in the electrode particles, $t$ is
time, $r$ is the radial distance, $D_p$ is the Li diffusivity in
\nca{}. The subscript $k$ represents the particle number from hereon.
...
particles are set to have an initial concentration of $c_p^0$.
...
\vg{units?}{$\delta = \SI[parse-numbers=false]{\sqrt{2}}{\micro\meter}$} .
...
\begin{figure}
  ...
  \caption{Evolution of $\langle x \rangle$ for the 8-particle system obtained using
    the tradition function of \gls{ecd}. All the particles delithiate
    at the same rate.}

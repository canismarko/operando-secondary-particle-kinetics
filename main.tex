...
\begin{abstract}
  ...
  the experimental results, an increase in \gls{ecd} over three orders
  of magnitude is required during delithiation.  The specifics and
...
containing highly oxidized \ce{Ni}. Physics-based simulations were
...
and \zref{si:fig:xrd-echem}). In all cases, the pristine state matched
...
higher $Q$ (lower $d$) upon removal of \ce{Li} (Figure
...
While the peak positions of \gls{p2} (\zref{si:fig:xrd-echem}b) underwent
...
\zref{si:fig:cell-pars}). Both particles reached cell parameters at
...
in Figures \ref{fig:uxrd}a,b and \zref{si:fig:ind-peaks}. Since \gls{p2}
...
content in each \nca{x} particle obtained from the \emph{c} parameter
...
(Figure \zref{si:fig:rates}). To allow for easier comparison between
...
\ce{Li_{0.9}Ni_{0.80}Co_{0.15}Al_{0.05}O2}, the reaction dramatically
accelerated nearly 10-fold, to \SI{\approx0.3}{\ce{Li}\per\hour}, or
C/3, until an average composition \nca{0.5}, followed by renewed
deceleration, to \SI{0.1}{\ce{Li}\per\hour}, or C/10, and rather small
...
similarly, with an 20-fold acceleration at \nca{0.9} to
\SI{\approx0.66}{\ce{Li}\per\hour}, or C/1.5, which lasted until an
average particle composition \nca{0.5}, where a slowdown to below
\SI{0.1}{\ce{Li}\per\hour}, or C/10, took place for the remainder of
...
\ce{Li_{0.8}Ni_{0.80}Co_{0.15}Al_{0.05}O2}, the rates slowed down to
\SI{\approx0.025}{\ce{Li}\per\hour}, or C/40. The behavior was
...
charging, which probed \ce{Ni} oxidation states at high spatial
...
the associated whiteline (Figures \zref{si:fig:isobestic-point} and
\zref{si:fig:kedges}). For layered transition-metal oxide cathodes,
the energy of maximum absorbance, or whiteline, of the \ce{Ni} K-edge
increases as \ce{Ni^{3+}} oxidizes to \ce{Ni^{4+}}. Therefore,
whiteline energy was used as a proxy for \ce{Ni} oxidation, which is
...
Initially, the whiteline energies were constant, indicating no \ce{Ni}
...
($\propto\sqrt{x(1-x)}$ \cite{newman1993, newman1994, newman1995,
  newman1996}) is used, as shown in Figure \todo{xx} in the Supporting
Information.
...
measurements (see Figure \todo{xx} in the Supporting Information) show that

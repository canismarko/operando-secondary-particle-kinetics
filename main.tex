...
rates at intermediate contents of \ce{Li} in \nca{x}, \mw{I don't
  understand this last sentence.}{qualitatively consistent with the
  oxidation.}
...
other C-rates are provided in \mw{Add crossref from SD}{Section}
...
found in \mw{Add crossref from SD}{Section} \ref{si:sec:}.
...
remaining results are provided in \mw{Add crossref from SD}{Section}
\ref{si:sec:}.
...
applied current increased (\mw{Add crossref from SD}{Section}
\ref{si:sec}).

Similar analyses were conducted with respect to $X_{\rm{Li}}^{\star}$
and $\Delta X_{\rm{Li}}$. The results are provided in (\mw{Add
  crossref from SD}{Figure} \ref{si:fig:}. These analyses revealed
...
information about the simulation setup can be found in \mw{Add
  crossref from SD}{Section} \ref{si:sec:}.
...
in \ce{LiCoO2} single crystal particles through \gls{iscat}, though
attributed this behavior to changes in \ce{Li^+} diffusion rather than
...
described here (\mw{Add reference to relevant models showing
  heterogeneous diffusion}{Figure} \ref{fig:}) both predict spatial
...
\begin{figure}
  ...
  \caption{Log scale comparison of different $i_0$ functions used in
    ...
    obtained from \todo[inline]{sort out these
      references} Ref.\ 9, Ref.xx, respectively, whereas plots labeled with

...
\begin{abstract}
  ...
  the electrode. This approach is subject of misleading analyses
  controlled by idiosyncratic kinetic conditions, such as surface
  ...
  particle agglomerates in layered cathodes during charge. After an
  ...
  relationship between Li content and cell parameter to reveal that
  rate acceleration systematically occurs between \gls{xLi} $\approx
...
wherein only \SI{\approx70}{\percent} is realistically achievable, a
...
surpass current bottlenecks. In particular, careful observation of the
behavior of individual particles, to bypass confounders from local
heterogeneity at the level of the electrode ensemble, combined with
detailed simulation of the underlying electrochemical processes, can
reveal details of the factors governing these reactions, ultimately
...
than the \ce{Li+} diffusion limitation. In the same report, ex-situ
...
delithiation in different layered oxides to sluggish \ce{Li^+}
...
the transport of \ce{Li+} can be studied using the

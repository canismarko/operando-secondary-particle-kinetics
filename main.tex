...
\begin{abstract}
  ...
  that, to reproduce the experimental results, \gls{ecd} must depend
  on \gls{xLi}, and that \gls{ecd} should increase rapidly over three
...
\mw{Include this paper: https://pubs.acs.org/doi/pdf/10.1021/acs.nanolett.2c01818}{}
\mw{Include this paper: https://onlinelibrary.wiley.com/doi/10.1002/anie.202205394}{}
...
cathode materials for electrochemical energy storage. In order to
...
\vg{The overall discussion [in these 2 paragraphs] gave me an
  impression that the Li2CO3 is the main reason behind the two-phase
  transformation. We might want to highlight that shortcomings of this
  explanation a bit more clearly.}{Ensemble} \gls{xrd} has shown a
...
lithium diffusion barrier arising from surface \ce{Li2CO3} formation
upon exposure to \ce{CO2} and \ce{H2O}. When pristine \nca{} was
...
contained an uneven coating of \ce{Li2CO3}, which must be broken
...
containing highly oxidized \ce{Ni}. We refer to this phenomenon rapid
...
\zref{si:fig:xrd-echem}). In all cases, the pristine state matched the
...
literature\cite{novak2015}, but \mw{differed from what}{their
  correlation with potential differed}. The lattice parameters for
...
reactions involving solid solutions, \vg{I think we should provide a
  ...
  in the introduction.}{}\kt{I don't understand this sentence.  It's
  not that homogeneity removes the barrier.  The thermodynamic free
  energy of a solid solution systems do not possess such a barrier, so
  nothing to ``remove''. [MW: maybe a question for Brian]}{where
  homogeneity removes the activation barrier toward nucleation of
  two-phase mechanisms.}
...
study was by Robert et al\cite{robert2015}.\ \kt{Are you demonstrating
  this in this work, or is this from Robert et al?  If latter, I would
  combine this sentence with the previous one by …, whose relationship
  holds…. (MW: Another Brian question)}{This relationship holds as
  long as single phases are observed}, as in Figure
...
\zref{si:fig:xrd-echem}). Figure \ref{fig:uxrd}e shows the \ce{Li}
content obtained from the \emph{c} lattice parameter for each of the
two \nca{x} particles during the first charge-discharge cycle. We
selected data points for which \kt{This statement does not seem to be
  consistent with the plot.  There are points where changes are very
  little from a point to another.  Maybe it's not “change” but
  “value”?}{reliable changes in lattice parameters were observed},
...
during charging to probed \ce{Ni} oxidation states at high spatial
...
\zref{si:fig:isobestic-point} and \zref{si:fig:kedges}). For layered
transition-metal oxide cathodes, energy of the the \ce{Ni} K-edge
whiteline increases as \ce{Ni^{2+}} oxidizes to \ce{Ni^{3+}} and then
\ce{Ni^{4+}}. Therefore, the whiteline energy was used as a proxy for
...
all particles, reaching \SIrange{75}{100}{\percent} \kt{When working
  ...
  better to stick with site fraction. [MW: I agree, we need to find
    ...
    fraction]}{state-of-charge}. Although some particles oxidized
concurrently, there was no clear preference for \vg{}{when particles
  began rapid oxidation}. Not all of the particles measured reached
...
exhibited \ce{Ni} reduction, presumably due to the instability of
...
(\SI{25}{\kilo\electronvolt}) X-rays, \kt{Where do you we discuss the
  effect of this parameter? Please clarify.}{and with varying carbon
  content in the electrode composite}. The persistence of this
...
\kt{Do we know that this is generically true?  The work examined two
  ...
  equates to a reduction in the usable capacity of the battery.} Both

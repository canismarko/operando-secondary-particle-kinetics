...
\begin{abstract}
  Most research on the electrochemical dynamics in materials for high-energy Li-ion batteries
  has focused on the global behavior of the electrode. This approach is subject of misleading analyses controlled by idiosincratic kinetic conditions, such as surface impurities inducing an apparent
  two-phase transformation within \nca{}. The work presented here uses nano-focused X-ray probes to
  measure delithiation \textit{operando} at the scale of secondary
  particle agglomerates in layered cathodes during
  ...
  potential. This behavior reproduces across \nca{} and \nmc{} compositions, irrespective of their reported tendency to display apparent
  two-phase transitions for the ensemble of the electrode. Operando uXRD leverages the relationship between Li content and cell parameter to reveal that rate acceleration systamtically happens between $\gls{xLi} \approx 0.9$ and $\approx 0.4$ for \nca{}. Physics-based modeling shows
  ...
  orders of magnitude at the transition point. The specifics and
...
maintaining cycling stability requires lower charge cut-off potentials wherein
only \SI{\approx70}{\percent} is realistically achievable, a challenge aggravated at higher rates of charge and
discharge\cite{janek2019-2,chen2020-4}. Fundamentally, these limitations are underpinned by the interplay between thermodynamic and kinetic relationships in the chemical transformations required for full utilization of the cathode. Thus, defining these intrinsic relationships remains foundational to the design of batteries that surpass current bottlenecks. In particular, careful
observation of the behavior of individual particles, to bypass confounders from local heterogeneity at the level of the electrode ensemble, combined with detailed simulation
of the underlying electrochemical processes, can reveal details of the
factors governing these reactions, ultimately leading to
...
behavior rather than the \ce{Li+} diffusion limitation. In the same
...
\ce{LiCO2} single crystal particles through \gls{iscat}. However, the
authors attributed this behavior to sluggish \ce{Li^+} transport in the Li-rich phase. Similarly, Ge et al.\cite{wang2020-6} showed rapid
...
corresponding to \Li{0.8}, the rates fell to
...
whiteline energies were extracted from the fit parameters. This approach provided higher
...
predicted in lithium iron phosphate \citeme{VG/KT?}, it does not apply
...
reported in literature\citeme{VG/KT?} and constructed for this work to
qualitatively reproduce rapid delithiation. The details of these forms
are provided in the Supporting Information. %% Our sensitivity
analyses generated
%% two
...
as well as the step form of \gls{D_s} having \todo{XXX} smaller value
(\vg{I will make this edit post the revision from Mark and Jordi. }{or
  something like this -- Vishwas, please edit}). Hence we can rule out
...
(8-particle), we simulate the (1) transport of \ce{Li+} in the
electrolyte, (2) transport of electrons in the cathode, (3) transport
of \ce{Li+} in the active material particles, and (4) electrochemical
...
this work, as listed in Table \zref{si:tab:model-parameters} and

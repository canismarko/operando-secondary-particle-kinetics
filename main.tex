...
available (Figure \ref{fig:txm-nca}d), whiteline energies were
compared to the lowest value observed for any particle in the field of
view during the operando experiment, reported here as
\textDelta{}whiteline. A parametric function was fit to each pixel's
full spectrum, and the whiteline energies were extracted from the fit
parameters. This approach provided higher precision and was more
tolerant of noise between frames, allowing for faster time
...
\todo{Write a transition paragraph that sets up the need for simulations.}
...
galvanostatic charging of the electrode, we performed electrochemical
dynamics simulations. In particular, we conducted particle-level
simulations of an \nca{}/Li cell with 8 particles, ranging from
\SIrange{4}{11}{\micro\meter} in diameter.

The electrodes used in the experiments were \vg{@Mark, please confirm
  this.}{highly porous} and had high carbon additive
content. Furthermore, the experimental C-rate was set to
\SI{0.05}{\ampere\per\ampere\per\hour}. Under these conditions, it is
reasonable to assume that the no electrolyte transport limitations and
electronic conduction limitations exist in the system. Therefore, the
accelerated delithiation could either be attributed to the exchange
current density, $i_0$, at the NCA/electrolyte interface or Li
diffusivity in NCA, $D_S$.

To further narrow down the underlying cause, we performed sensitivity
analyses with respect to both $i_0$ and $D_S$ at different
C-rates. The results for the sensitivity analyses at C/20 are
summarized below, while the results at other C-rates are provided in
the \mw{ref to specific section}{Supporting Information}. More details
about the model, including the governing equations and the complete
set of parameters employed, can also be found in the \mw{ref to
  specific section}{Supplementary Information}.

%% sensitivity analysses with respect to i_0

In this section, we report the results of the sensitivity studies with
respect to different forms of $i_0$. We note that we set
...
$X_{\rm{Li}}$ is the Li site fraction in the active
...
$X_{\rm{Li}}$\cite{chueh2021}. Other studies show that $i_0$ is a
...
known. Furthermore, the literature is missing the data for
$X_{\rm{Li}} > 0.92$.

To work with the uncertain form of $i_0$, we ran two sets of
simulations. In the first set of the simulations, we employed the
traditional form of $i_0$, which is shown in Figure
\ref{fig:i0_profiles}. In the second set of simulations, we used a
model form of $i_0$, which is also shown in Figure
\ref{fig:i0_profiles}. The model form is setup to have a smooth step
function of $X_{\rm{Li}}$; the step in the function is denoted by a
cubic spline.

The traditional form only has the max value of $i_0$,
$i_{0,\rm{max}}$, as a tunable parameter, whereas the model form has
four parameters: $i_{0,\rm{min}}$, the minimum value of $i_0$,
$i_{0,\rm{max}}$, $X_{\rm{Li}}^?$, the value of where the transition
begins from $i_{0,\rm{min}}$ to $i_{0,\rm{max}}$, and $\Delta
X_{\rm{Li}}$, the width of the transition zone. The presence of
additional parameters enables the model function of $i_0$ to represent
most of the aforementioned-literature-reported functions. Since the
values of $i_{0,\rm{max}}$ reported in the
literature\cite{tsai2018,dees2008} do not vary widely, we selected an
intermediate value, $i_0 = \SI{1}{\ampere\per\meter\squared}$. The
comparison of the literature values, traditional and model functions
of $i_0$ is provided in Figure \ref{fig:i0_profiles}.

%% Effects of i_0 on the dilithiation dynamics
The evolution of the volume average Li-site fraction,
$\left<X_{\rm{Li}}\right>$, for the first set of simulations, i.e.,
with the traditional form of $i_0$ at \SI{0.05}{\per\hour} are
provided in Figure \ref{fig:i0_basic}. As can be seen, for this case,
none of the particles exhibit the accelerated delithiation behavior,
and all the particles delithiate at the same rate.
...
$i_{0,\rm{min}}$, $X_{Li}^?$, and $\Delta X_{\rm{Li}}$. The results
corresponding to $i_{0,\rm{min}}$ are provided here and the remaining
results are provided in the \mw{ref to specific section}{Supporting
  Information}.

We chose four different values of $i_{0,\rm{min}}$ ranging from
\SIrange{0.01}{0.001}{\ampere\per\meter\squared}, while setting the
values of other parameters as $i_{0,\rm{max}} =
\SI{1}{\ampere\per\meter\squared}$, $X_{\rm{Li}}^{\star} = 0.9$, and
$\Delta X_{\rm{Li}} = 0.05$.  The comparison of different $i_0$
functions that differ in terms of $i_{0,\rm{min}}$ is provided in
Figure \ref{fig:i0_profiles}. The corresponding results for the
evolution of $\left<X_{\rm{Li}}\right>$ are provided in Figure
\ref{i0_models}. It can be seen that all particles exhibit accelerated
delithiation for all values of $i_{0,\rm{min}}$. Furthermore, it can
be observed that as $i_{0,\rm{min}}$ is decreased, the
$\left<X_{\rm{Li}}\right>$ value at which the accelerated delithiation
of each particle halts also decreases, which will hereafter be denoted
as $\left<X_{\rm{Li}}\right>^\dagger$. Since the $i_{0,\rm{min}}$
value of \SI{0.001}{\ampere\per\meter\squared} resulted in a similar
value of $\left<X_{\rm{Li}}\right>^\dagger$ as the experiments, we
selected this $i_{0,\rm{min}}$ value as the nominal value.

The oscillations in $\left<X_{\rm{Li}}\right>$ in a few cases
originate because of the large difference between $i_{0,\rm{min}}$ and
$i_{0,\rm{max}}$, that causes a particle undergoing the transition
between these values to deliver more than the applied current, which
induces other more delithiated particles to lithiate slightly. Whether
such oscillations occur in a physical system is difficult to determine
as our experimental measurements have limited chemical and temporal
resolution. These oscillations reduce in amplitude in a system with
larger number of particles (as shown in later sections). We also
observe a reduction in the amplitudes as the C-rate is increased at
\vg{}{(see section xx in the Supporting Information)}.
...
and $\Delta X_{\rm{Li}}$. The results are provided in the \mw{ref to
  actual SI section}{Supporting Information (Figure xx)}. In
summary, it was found that $X_{\rm{Li}}^{\star}$ controlled the onset
of the accelerated delithiation, for instance, for
$X_{\rm{Li}}^{\star} = 0.8$, the accelerated delithiation in a
particle would occur at $\left<X_{\rm{Li}}\right> = 0.8$. Whereas
$\Delta X_{\rm{Li}}$ controlled the rate of accelerated delithiation;
larger $\Delta X_{\rm{Li}}$ resulted in smaller
$\frac{d\left<X_{\rm{Li}}\right>}{dt}$ and vice versa.

The results from the sensitivity analyses show that the $i_0$ function
must have a sufficiently large difference between $i_{0,\rm{min}}$ and
$i_{0,\rm{max}}$, along with a sufficiently sharp transition between
these two values in order to reproduce the accelerated delithiation
observed in experiments. Such a function enables a particle that has a
composition in the transition zone from $i_{0,\rm{min}}$ to
$i_{0,\rm{max}}$, to rapidly delithiate and deliver increasing amount
of current as $i_0$ increases rapidly. This acceleration causes the
particle to contribute majority of the applied current, thereby
resulting in a substantial decrease in the delithiation rate of the
other particles, which have a smaller $i_0$ value. By the time the
particle ends its accelerated delithiation, another particle (mostly
likely the next largest particle) undergoes this transition, and thus,
accelerated delithiation. This cycle repeats till the last particle
undergoes accelerated delithiation.

%% Sensitivity analyses with respect to $D_s$

For the sensitivity analyses with respect to $D_s$ , we selected two
forms of $D_s$ : a) the literature reported form, which was obtained
from \vg{}{Ref xx. [ref]} and b) the model form. Both the forms are
shown in \vg{}{Figure xx}. It should be noted that the model form is
similar to the smooth step functional form used for $i_0$ . Furthermore,
the traditional form of $i_0$ is used for the simulations in this
section.

The results for the delithiation dynamics for both the cases are
presented in \vg{}{Figure xx}. As can be seen, no accelerated
delithiation is observed in both cases. This is because for a particle
to exhibit accelerated delithiation, the flux at the
...
$X_{\rm{Li}}$ (the model form) because even with such a dependence,
the diffusivity (hence, diffusion flux) only increases in the shell of
the particle close to the particle surface; the particle bulk
continues to have the low diffusivity value. In other words, the
entire particle does not undergo rapid delithiation; instead, only the
shell close to the particle surface exhibits such behavior, as shown
in \vg{}{Figure xx}.

%% \subsection{Large-Scale Simulation}

To confirm that the conclusions made in the previous section hold
valid for a larger system, we performed a simulation at C/20 for a
30-particle system using the model function of $i_0$, for which the
parameters were identified above. Furthermore, the simulation was
configured to follow the experimental setup where possible, including
the particle-size (diameter) distribution as well as the configuration
of the electrode. More information about the simulation setup can be
found in the \mw{ref to actual section}{Supporting Information}.
...
$\left<X_{\rm{Li}}\right>^\dagger$ also reduced significantly for the
30-particle system. Thus, we conclude that our analyses above also
stand valid for larger ensembles of particles, which represent the
experimental conditions more closely.

\vg{We can remove/modify this para if we feel that it dilutes our
  message.}{Finally}, we note that these analyses are only applicable
for low C-rate operations. At high C-rates, there exist electrode
level inhomogeneities in the electrochemical reaction distribution due
to electrolyte transport limitation\cite{dasgupta2020} or electronic
conduction limitation\cite{liu2019-2}. Under such conditions, the
delithiation behavior of particles would be influenced by their
position in the electrode.  Furthermore, the presence of a
sufficiently thick cathode electrolyte interphase layer can also alter
the order in which the particles delithiate. All these effects can be
captured by properly developing an electrode level model, which is
beyond the scope of this study.

%% \subsection{Conclusion of the Modeling Study}

Based on the insights generated above, we conclude that the
accelerated delithiation in \nca{} particle is controlled by the
surface reaction kinetics instead of bulk Li diffusion dynamics in the
active material. Moreover, we need $i_0$ to have a large ($\frac{
...
for each quantity and should not be considered as
precise. Furthermore, we conclude that the results obtained from an
8-particle model are still valid for a larger system with 30
particles, which is more representative of the actual electrode used
in the experiments.

The results from our modeling study agrees well with the report by
Park et al.\cite{chueh2021}, where the authors showed that the
accelerated delithiation or electro-autocatalysis, in their terms, is
controlled by the surface reaction and not the solid-state
diffusion. Furthermore, the authors also reported a rapid increase in
the exchange current at the \nca{} particle surface as the
delithiation progresses.
